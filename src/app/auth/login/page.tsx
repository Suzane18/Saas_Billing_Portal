...
export default function LoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (res?.error) {
      const errorMessage = res.error || 'An unexpected error occurred. Please try again.'
    ...
    // Wait for toast animation and then redirect
    setTimeout(() => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-white">
      ...
      {showSuccess && <Toast message="Login successful" type="success" duration={2000} />}
    </div>
  )
}
